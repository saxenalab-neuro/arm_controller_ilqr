#!/bin/sh
#SBATCH --job-name=armoptmp
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=24:00:00
#SBATCH --array=1-8
#SBATCH --output=RSAC_m_p_%A-%a.out

# Purpose: generate proprioception-only movement trajectories from the matching
# proprioception-only fixation checkpoint.
#
# Prerequisite: RSAC_f_p.sh must finish successfully. Recommended command:
#   sh submit_all.sh
...
test -f "$data_root/fixation_p/xk1_init_move_${SLURM_ARRAY_TASK_ID}.npy"
command -v "$python_bin" >/dev/null 2>&1

# Run proprioception-only movement for this array index.
"$python_bin" "$repo_root/ilqr_m_p.py" --final-theta "$repo_root/final_theta_traj.npy"

#!/bin/sh
#SBATCH --job-name=armoptmv
#SBATCH --ntasks=1
#SBATCH --cpus-per-task=4
#SBATCH --mem=16G
#SBATCH --time=24:00:00
#SBATCH --array=1-8
#SBATCH --output=RSAC_m_v_%A-%a.out

# Purpose: generate vision-only movement trajectories from the matching
# vision-only fixation checkpoint.
#
# Prerequisite: RSAC_f_v.sh must finish successfully. Recommended command:
#   sh submit_all.sh
...
test -f "$data_root/fixation_v/xk1_init_move_${SLURM_ARRAY_TASK_ID}.npy"
command -v "$python_bin" >/dev/null 2>&1

# Run vision-only movement for this array index.
"$python_bin" "$repo_root/ilqr_m_v.py" --final-theta "$repo_root/final_theta_traj.npy"
